...
axios.interceptors.response.use(async response=>{
    ...
    if(pagination){
        response.data = new PaginateResponse(response.data, JSON.parse(pagination));
...
const Basket={
    ...
    addItem: (productId:number, kolicina=1)=>requests.post(`basket?productId=${productId}&kolicina=${kolicina}`, {}),
    removeItem:(productId:number, kolicina=1)=>requests.delete(`basket?productId=${productId}&kolicina=${kolicina}`)
}
...
const Reservations ={
    list:() => requests.get("orders/reservation"),
}
