...
function Item(order: any){
  ...
  return(
    <TableCell>
      <div>
        <label>
          <input style={{color:"green", marginLeft:"-200px", tabSize:"15px",}} type="checkbox" checked={returned} 
          onChange={()=>{
            setReturned((r:any)=>!r);
...
export default function BasketTable({items, isBasket=true,  isOrder}:Props){
            ...
            {items.map((item) => (
              ...
              >
                ...
                <TableCell align="right">
                  {item.cijena*item.kolicina*(dayjs(dateTo).diff(dayjs(dateFrom))/86400000+1)}KM
                  </TableCell>
